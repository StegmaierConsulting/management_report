...
import RootCauseAnalysisFormEdit, { DatosExtraidos, TableData } from '@/components/RootCauseAnalysisFormEdit';
...
const SearchForms: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    const collectionPath = `USERAUTH/${selectedEmpresa}/RootCauseAnalysis`;
    ...
    if (q) {
      ...
      querySnapshot.forEach((doc) => {
        ...
        // Suposición: los documentos tienen datos con claves de "1", "2", "3", etc.
        Object.keys(data).forEach(key => {
          if (key !== "empresaGuardado" && key !== "numeroDocumento" && key !== "timestamp") {
            tables[key] = data[key];
          }
        });
        ...
        forms.push({
          tables: tables,
          numeroDocumento: data.numeroDocumento // Incluye la información adicional si es necesaria
        });
      });
      setResults(forms);
    }
        ...
        <div>
          ...
          {selectedForm && (
            <div className="mt-4">
              ...
              <RootCauseAnalysisFormEdit
                formData={selectedForm.tables}
                handleChange={(e, tableId) => {
